...
你可能会有一个`疑问`：为什么`main`函数不需要添加`pub`注解就能调用？这个问题我们可以在编译器的源码住观察到：
```rust
...
## `pure`
相信了解过`函数式变成`的读者一定知道一个概念：`纯函数`，即`无副作用函数`，也就是说，调用一个`纯函数`，相同的输入永远得到相同的输出，并且不会`产生I/O操作`，`修改全局状态`，`读取外部状态`。
...
```
然后编译运行，经过测试，在`笔者`的笔记本电脑中的运行时间大约在`90ms`，但对于这种固定输入得到固定输出的程序，我们只需要稍作修改：
```alum
$import "io.ah"
...
在`Alum 0.9.6中`，`GosVM`已经回归，但并非是一个`完整的后端`，而是作为`受限的子集`，在`编译时`将无副作用的`函数`直接计算出结果，提升`运行时`性能。