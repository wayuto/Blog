...
```
我们在[09-外部函数接口](09-FFI.md)学到的`extern`就是其中之一，目前，`Alum 0.9.7`仅支持`extern`，`pub`与`pure`三种，我们将介绍`pub`与`pure`的用途。  
...
但读者肯定也能明显感知到`编译时`时间显著增加（如果你使用的是`Alum 0.9.8`，那么可能不明显，详情阅读[18-编译时求值](18-CTE.md)）。  
